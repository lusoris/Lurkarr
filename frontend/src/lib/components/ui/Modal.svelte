...
{#if open}
	<div class="fixed inset-0 z-50 flex items-center justify-center">
		<!-- Backdrop -->
		<button class="absolute inset-0 bg-black/60 backdrop-blur-sm" onclick={onclose} aria-label="Close" />
		...
		<div class="relative z-10 w-full max-w-lg rounded-xl border border-surface-700 bg-surface-900 p-6 shadow-2xl">
			{#if title}
				<div class="flex items-center justify-between mb-4">
					...
					<button onclick={onclose} class="text-surface-400 hover:text-surface-100 transition-colors">
